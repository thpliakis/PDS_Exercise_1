...
int main(int argc, char *argv[]){
    ...
    double *val;
    uint32_t num=0;
    char workers[6][3] = {"1", "2", "5", "8", "10", "20"};
    ...
    if ((f2 = fopen("cilk_runtimes.csv","a")) == NULL){
    ...
    for (uint32_t l = 0; l < M; l++) vector[l] = 0;           //  Intialize with 0s vector
    c3 = (uint32_t *)malloc(M   * sizeof(uint32_t));
    for (uint32_t l = 0; l < M; l++) c3[l] = 0;              //  Intialize with 0s c3
    ...
    for(int w=0; w<6; w++){
        ...
        // and c3 contains in how many extra triangles a node takes part in, which is 2 more times.
        for(int col = 0; col < M; col++){
            //printf("%d  %d\n",c3[col],vector[col]);
            c3[col] += vector[col];
        }
        ...
        // To get the number of workers
        printf("Number of threads = %d\n",getWorkers());
        printf("Counting triangles wall clock time: %f sec\n", par_time);      // print excecution time
        printf("triang_number = %d\n",num);                                    // print the number of triangles found in the graph


        fprintf(f2,"%s,%s,%d,%d,%f,%d\n",argv[0],argv[1],M,getWorkers(),par_time,num);  // write times to csv file
